...
### Ambient Background
```css
.ambient-bokeh{position:fixed;inset:0;z-index:-1;pointer-events:none;opacity:.34;}
```
...
The background renders a small number of soft circles with opacity and scale only, pauses when the document is hidden, and falls back to a static gradient on mobile.
...
| Breakpoint | Behavior |
|------------|----------|
| Desktop >900px | 8–10 slow lights, opacity .34 |
